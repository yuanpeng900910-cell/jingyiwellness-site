...
function renderStatic() {
  ...
  quickGuide.innerHTML = `<article class="base-card"><h2>00 销售人员快速使用指南</h2><h3>三步推荐法</h3><ul>${data.quickGuide.steps.map(s=>`<li>${s}</li>`).join('')}</ul><h3>统一基础口径</h3><ul>${data.quickGuide.basics.map(s=>`<li>${s}</li>`).join('')}</ul></article>`;
}
